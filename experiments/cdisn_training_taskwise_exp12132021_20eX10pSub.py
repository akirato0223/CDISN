...
def set_up_and_run_current_experiment(args):
    ...
    for i, task_hyperparams in enumerate(zip(tasks_to_parallelize, data_for_tasks)):
        ...
        for dataset_path in list(main_datasets_for_requested_tasks):
            ...
            train_sets = sorted([x for x in os.listdir(dataset_path) if "train" in x and '.pkl' not in x]) # os.path.isdir(os.path.join(dataset_path, x))])
            val_sets = sorted([x for x in os.listdir(dataset_path) if "val" in x and '.pkl' not in x]) # os.path.isdir(os.path.join(dataset_path, x))])
            test_sets = sorted([x for x in os.listdir(dataset_path) if "test" in x and '.pkl' not in x]) # os.path.isdir(os.path.join(dataset_path, x))])
            if len(test_sets) == 0:
                test_sets = [None]
    ...
    if len(task_param_settings[5]) == 3:
        kick_off_training_run(full_parameter_dict["cdisn"], task_param_settings)
    else:
        print("<<< NOT ENOUGH REQUESTED TASKS - SKIPPING TASKID == ", taskID)
    return taskID
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-cached_args_file",
        type=str,
        default="cached_args_cdisn_training_taskwise_exp12172021_20e3_10pSub3_RPTSBTUAB.pkl",
